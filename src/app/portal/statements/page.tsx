  ...
  return (
    ...
    >
      ...
      <div className="border border-[#E0E0E0] bg-white" data-testid="statements-list">
        ...
        {STATEMENTS.map(s => (
          <div key={s.id} className="border-b border-[#F0F0F0] last:border-b-0 px-4 sm:px-6 py-4 grid grid-cols-2 md:grid-cols-12 gap-3" data-testid={`statement-${s.id}`}>
            <div className="md:col-span-3">
              <span className="block text-[#0A0A0A] font-medium" style={{ fontSize: 14 }}>{s.period}</span>
              <span className="label">{s.id}</span>
            </div>
            <div className="md:col-span-3"><span className="tabular text-[#6B6B6B]" style={{ fontSize: 13 }}>{s.generated}</span></div>
            <div className="md:col-span-3 md:text-right"><span className="tabular text-[#0A0A0A]" style={{ fontSize: 14 }}>{fmt(s.net)}</span></div>
            <div className="md:col-span-3 md:text-right">
              {s.status === 'ready' ? (
                <a
                  href={s.downloadUrl}
                  className="label tap-target px-3 inline-flex border border-[#0A0A0A] text-[#0A0A0A] hover:bg-[#0A0A0A] hover:text-white transition-colors"
